                    ...
                    if i_line.product_id.type != 'service':
                        ...
                        for line in res:
                            if a == line['account_id'] and i_line.product_id.id == line['product_id']:
                                ...
                                if get_price(cr, uid, inv, company_currency, i_line) != line['price'] and acc:
                                    price_diff = line['price'] - get_price(cr, uid, inv, company_currency, i_line)
                                    line.update({'price':get_price(cr, uid, inv, company_currency, i_line)})
                                    diff_res.append({
                                        'type':'src',
                                        'name': i_line.name[:64],
        ...
        if invoice.type == 'in_invoice':
            ...
            for _, _, line_dict in invoice_data['invoice_line']:
                if line_dict.get('product_id'):
                    ...
                    counterpart_acct_id = product.property_stock_account_input and \
                            product.property_stock_account_input.id
                    if not counterpart_acct_id:
                        counterpart_acct_id = product.categ_id.property_stock_account_input_categ and \
